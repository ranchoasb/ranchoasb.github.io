...
//displaying info on index page
  ...
  //carousel (rotating images)
      if (carousel){
        ...
        description += `</div><div class="carousel-inner"><div class="carousel-item active" data-bs-interval="3000"><iframe src="${carouselImgs[0]}" style="margin:auto" height="333.75" class="d-block" alt="..."></iframe></div>`;
        for (let i = 1; i < carouselImgs.length; i++){
    ...
    desiredHeight = 333.75; // Get the width of the active item
    activeWidth = (activeItem.offsetWidth)/(activeItem.offsetHeight)*(desiredHeight); // Calculate the desired width
    }
  ...
  fetch("https://script.google.com/macros/s/AKfycby9qWGns9f47zJmJnyE9gNYCy-DtuqcsE68ClRIiaCGstEuBAQZgdGUstkqCFeFGMFf/exec?query=upcomingEvents").then(e=>e.text()).then(response=>{
