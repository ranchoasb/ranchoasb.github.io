window.onload = () => {
  ...
  function load() {
    ...
    for (let index in rows) {
      ...
      let date = row.time=="N/A" ? "N/A" : new Date(row.time).toLocaleDateString();

      let description = row.description;
      let category = row.category;
      let trigger = +row.trigger;
      
      loadmorebutton.insertAdjacentHTML("beforebegin", `<div class="card-link" ${trigger ? `data-bs-toggle="modal" data-bs-target="#rowmodal${index}"` : ""} id="x${title.replaceAll(" ", "-")}"><div class="card"><div class="card-header"><div class="d-flex align-items-center justify-content-between"><div class="d-flex align-items-center"><div>${title=="N/A" ? "" : `<h6 class="card-title mb-0">${title}</h6>`}${date=="N/A" ? "" : `<p class="small mb-0">${date}</p>`}</div></div></div></div><div class="card-body">${description=="N/A" ? "" : `<div class="mb-0">${description}</div>`}</div><div class="card-footer border-0 d-flex justify-content-between align-items-center">${category=="N/A" ? "" : `<p class="mb-0">Category: ${category}</p>`}</div></div></div><div class="modal" tabindex="-1" id="rowmodal${index}"><div class="modal-dialog"><div class="modal-content"><div class="modal-header"><h5 class="modal-title">${title} <small>on ${date}</small></h5><button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button></div><div class="modal-body"><p>${description}</p><p><small>Category: ${category}</small></p></div><div class="modal-footer"><button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button></div></div></div></div>`);
    }
