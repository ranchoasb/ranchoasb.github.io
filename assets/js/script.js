window.onload = () => {
  ...
  fetch("https://script.google.com/macros/s/AKfycbylp7XIw-zKcFtz1tOjPAI9_sR-I3PYyjP1bMXWTyrio3IYEMBNxeg2XT_1X9DzU_4H/exec?query=announcements").then(e=>e.text()).then(response => {
    ...
    for(let i=0;i<3;i++){
      announcements.innerHTML+=`<u><a href="${response2.slice(response2.indexOf("https://docs.google.com/presentation/d/"),response2.indexOf("sharing")+7)}">Announcements for ${response2.slice(response2.indexOf("data-date=")+11,response2.indexOf("/2023"))}</a></u><br>`
      response2.replace("https://", "");
      response2.replace("sharing", "");
      response2.replace("data-date", "");
      response2.replace("/2023", "");
    }
